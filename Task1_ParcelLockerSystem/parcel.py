...
class Parcel(ABC):
    """Abstract parcel class used by all parcel types."""
    ...
    def __init__(self, tracking_id: str, sender: str, recipient: str, weight: float):
        if not self.validate_tracking_id(tracking_id):
            raise ValueError("Tracking ID must start with 'P' and have at least 5 characters.")
        self._tracking_id = tracking_id.upper()
        self._sender = sender
    ...
    def validate_tracking_id(tracking_id: str) -> bool:
        return isinstance(tracking_id, str) and tracking_id.upper().startswith("P") and len(tracking_id) >= 5
    ...
    def __eq__(self, other: object) -> bool:
        if not isinstance(other, Parcel):
            return False
        return self.tracking_id == other.tracking_id
